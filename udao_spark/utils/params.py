...
#     """Holds the configuration for a single benchmark."""
#
#     def __init__(self, **kwargs) -> None:
#         if "name" not in kwargs:
#             raise ValueError("Missing 'name' key in benchmark config.")
#         self.name = kwargs["name"]
#         self.scale_factor = kwargs.get("scale_factor", 100)
#         self.data_percentage = kwargs.get("data_percentage", None)
#         self.data_amount = kwargs.get("data_amount", None)
#         self.is_extended = kwargs.get("is_extended", False)
#
#     def __repr__(self) -> str:
#         return f"BenchmarkConfig({self.__dict__})"
#
#
# def parse_benchmark_config(s: str) -> BenchmarkConfig:
#     parts = s.split(",")
#     config_dict = {}
#     for part in parts:
#         part = part.strip()
...
#         else:
#             raise ValueError(f"Invalid format in: '{part}'.
#             Expected key=value pairs.")
#
#     if "name" not in config_dict:
#         raise ValueError("Missing 'name' key in benchmark config.")
#     if "data_percentage" in config_dict and "data_amount" in config_dict:
#         raise ValueError(
#             "Both 'data_percentage' and 'data_amount' are set. Only one is allowed."
#         )
#
...
def get_base_parser() -> ArgumentParser:
    ...
    parser.add_argument("--fold_peek_percentage", type=int, default=0,
                        help="Portion number, from 0 to 90. ID is 90/10")
    ...
    parser.add_argument("--ext_up_to_n_joins", type=int, default=None,
                        help="number of max joins in the added sqls (for job/ds-sj)")
    parser.add_argument("--data_percentage2", type=int, default=None,
                        help="Portion number, from 0 to 100")
    parser.add_argument("--benchmark_ext2", type=str, default=None,
                        help="Benchmark extended")
    parser.add_argument("--ext_data_amount2", type=int, default=None,
                        help="number of sqls to extend")
    parser.add_argument("--ext_up_to_n_joins2", type=int, default=None,
                        help="number of max joins in the added sqls (for ds-sj)")

    # todo: refactor benchmarks
    # parser.add_argument("--multi-benchmark", action='append',
    #                     type=parse_benchmark_config,
    #                     help="Benchmark config as key=value pairs "
    #                          "(e.g., name=tpch,scale_factor=100)")
    # fmt: on
    return parser


def get_base_learning() -> ArgumentParser:
    # fmt: off
...
def wrap_sk_mlp_params(parser: ArgumentParser) -> ArgumentParser:
    # fmt: off
...
def get_graph_conv_net_params() -> ArgumentParser:
    parser = _get_graph_base_parser()
    # fmt: off
    # Embedder parameters
    parser.add_argument("--output_size", type=int, default=32,
                        help="Embedder output size")
    parser.add_argument("--gcn_n_layers", type=int, default=2,
