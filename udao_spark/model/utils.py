...
from .embedders.graph_conv_net import GraphConvNet
...
@dataclass
class GraphAverageSKMLPParams(GraphAverageMLPParams):
    use_batchnorm: bool = True
    activate: str = "relu"

    @classmethod
    def from_dict(cls, data_dict: Dict[str, Any]) -> "GraphAverageSKMLPParams":
        if "iterator_shape" not in data_dict:
            raise ValueError("iterator_shape not found in data_dict")
        if not isinstance(data_dict["iterator_shape"], UdaoEmbedItemShape):
            iterator_shape_dict = data_dict["iterator_shape"]
    ...
    def hash(self) -> str:
        ...
        sha256_hash = hashlib.sha256(attributes_tuple)
        hex12 = sha256_hash.hexdigest()[:12]
        return "graph_avg_sk_" + hex12


def get_graph_avg_sk_mlp(params: GraphAverageSKMLPParams) -> UdaoModel:
    model = UdaoModel.from_config(
        embedder_cls=GraphAverager,
...
@dataclass
class GraphConvNetSKMLPParams(UdaoParams):
    iterator_shape: UdaoEmbedItemShape
    op_groups: List[str]
    output_size: int = 32
    gcn_n_layers: int = 2
    ...
    @classmethod
    def from_dict(cls, data_dict: Dict[str, Any]) -> "GraphConvNetSKMLPParams":
        if "iterator_shape" not in data_dict:
            raise ValueError("iterator_shape not found in data_dict")
        if not isinstance(data_dict["iterator_shape"], UdaoEmbedItemShape):
            iterator_shape_dict = data_dict["iterator_shape"]
    ...
    def hash(self) -> str:
        ...
        sha256_hash = hashlib.sha256(attributes_tuple)
        hex12 = sha256_hash.hexdigest()[:12]
        return "graph_gcn_sk_mlp" + hex12


def get_graph_gcn_sk_mlp(params: GraphConvNetSKMLPParams) -> UdaoModel:
    model = UdaoModel.from_config(
        embedder_cls=GraphConvNet,
        regressor_cls=SkipConnectionMLP,
        iterator_shape=params.iterator_shape,
        embedder_params={
            "output_size": params.output_size,  # 128
...
@dataclass
class XFerSKMLPParams(UdaoParams):
    ...
    @classmethod
    def from_dict(cls, data_dict: Dict[str, Any]) -> "XFerSKMLPParams":
        return cls(**data_dict)
    ...
    def hash(self) -> str:
        ...
        sha256_hash = hashlib.sha256(attributes_tuple)
        hex12 = sha256_hash.hexdigest()[:12]
        return "xfer_sk_mlp" + hex12


def get_xfer_sk_mlp(params: XFerSKMLPParams) -> UdaoModel:
    iterator_shape = UdaoEmbedItemShape(
        embedding_input_shape=params.input_embedding_dim,
    ...
    return UdaoModel.from_config(
        ...
        embedder_params={"output_size": params.input_embedding_dim},
        regressor_params={
            "n_layers": params.n_layers,  # 3
            "hidden_dim": params.hidden_dim,  # 512
            "dropout": params.dropout,  # 0.1
...
def get_gat_sk_mlp(params: GraphTransformerSKMLPParams) -> UdaoModel:
    model = UdaoModel.from_config(
        embedder_cls=GAT_Transformer,
        regressor_cls=SkipConnectionMLP,
        iterator_shape=params.iterator_shape,
        embedder_params={
            "output_size": params.output_size,  # 128
...
def compute_metrics(
    obj_names: List[str], obj_df: pd.DataFrame
) -> Dict[str, Dict[str, float]]:
    metrics: Dict[str, Dict[str, float]] = {}
    for obj_ind, obj in enumerate(obj_names):
        metrics[obj] = {}
        y = obj_df[obj].values.astype(np.float32)
        y_pred = obj_df[f"{obj}_pred"].values.astype(np.float32)
        ...
        metrics[obj]["p99_wape"] = local_p99_wape(y, y_pred)
        metrics[obj]["max_wape"] = local_max_wape(y, y_pred)
...
def sink_job_stats(
    ...
    if not os.path.exists(
        ...
        for masks, test_type in [(id_mask, "SYNTHETIC"), (ood_mask, "LIGHT")]:
            ...
            metrics = compute_metrics(obj_names, obj_df_sub)
...
def sink_tpc_stats(
    ...
    if not os.path.exists(
        ...
        metrics = compute_metrics(obj_names, obj_df)
        print("metrics: ", metrics)
        test_file_name_sub = f"obj_df_test_{benchmark}_{device}.pkl"

        if "tpcds" == benchmark:
            header = ckp_learning_header.split("/cache_and_ckp")[0]
            select1 = read_list_from_file(
                f"{header}/assets/tpcds_templates/select1.txt"
            )
            select2 = read_list_from_file(
                f"{header}/assets/tpcds_templates/select2.txt"
            )
            dfq = ParquetHandler.load(
                f"{header}/cache_and_ckp/tpcds_102x490", "df_q_compile.parquet"
            )
            id2t = dfq["template"].to_dict()
            obj_df["template"] = id2t
            metrics_select1 = compute_metrics(
                obj_names, obj_df[obj_df["template"].isin(select1)]
            )
            metrics_select2 = compute_metrics(
                obj_names, obj_df[obj_df["template"].isin(select2)]
            )
            print("metrics (drop1): ", metrics_select1)
            print("metrics (drop2): ", metrics_select2)
            PickleHandler.save(
                {
                    "obj_df": obj_df,
                    "metrics": metrics,
                    "drop1_metrics": metrics_select1,
                    "drop2_metrics": metrics_select2,
                },
                ckp_learning_header,
                test_file_name_sub,
                overwrite=True,
...
def init_setup(params: Namespace) -> Tuple[str, th.dtype]:
    set_deterministic_torch(params.seed)
    # args.benchmark is now a list of BenchmarkConfig objects
    if params.benchmark:
        for i, benchmark_cfg in enumerate(params.benchmark, start=1):
            print(f"Benchmark #{i}: {benchmark_cfg}")
            if benchmark_cfg.name == "tpcds":
                th.set_float32_matmul_precision("medium")  # type: ignore
    print(params)
    device = "gpu" if th.cuda.is_available() else "cpu"
    tensor_dtypes = th.float32
    th.set_default_dtype(tensor_dtypes)  # type: ignore
    return device, tensor_dtypes
...
def param_init(base_dir: Path, params: Namespace) -> Tuple[TypeAdvisor, PathWatcher]:
    ...
    pw = PathWatcher(
        ...
        params.ext_up_to_n_joins,
        params.data_percentage2,
        params.benchmark_ext2,
        params.ext_data_amount2,
        params.ext_up_to_n_joins2,
        params.fold_peek_percentage,
    )
    return ta, pw


def train_and_dump_base(
    bm: str,
    ta: TypeAdvisor,
    model: UdaoModel,
    ckp_header: str,
    ...
    save_divide(bm, base_dir, n_test, ckp_learning_header, device, fold, objectives)

    if dump_query_embedding:
...
def train_and_dump(
    ...
    params: Namespace,
    device: str,
    dump_query_embedding: bool = True,
) -> None:
    ...
    train_and_dump_base(
        ...
        hp_params=hp_params,
        dump_query_embedding=dump_query_embedding,
        base_dir=str(pw.base_dir),
        fold=pw.fold,
    )
...
@dataclass
class TreeLSTMSKParams(TreeLSTMParams):
    use_batchnorm: bool = True
    activate: str = "relu"

    @classmethod
    def from_dict(cls, data_dict: Dict[str, Any]) -> "TreeLSTMSKParams":
        if "iterator_shape" not in data_dict:
            raise ValueError("iterator_shape not found in data_dict")
        if not isinstance(data_dict["iterator_shape"], UdaoEmbedItemShape):
            iterator_shape_dict = data_dict["iterator_shape"]
    ...
    def hash(self) -> str:
        ...
        sha256_hash = hashlib.sha256(attributes_tuple)
        hex12 = sha256_hash.hexdigest()[:12]
        return "tree_lstm_sk_" + hex12


def get_tree_lstm_sk_mlp(params: TreeLSTMSKParams) -> UdaoModel:
    model = UdaoModel.from_config(
        embedder_cls=TreeLSTM,
        regressor_cls=SkipConnectionMLP,
        iterator_shape=params.iterator_shape,
...
@dataclass
class TreeCNNSKParams(TreeCNNParams):
    use_batchnorm: bool = True
    activate: str = "relu"

    @classmethod
    def from_dict(cls, data_dict: Dict[str, Any]) -> "TreeCNNSKParams":
        if "iterator_shape" not in data_dict:
            raise ValueError("iterator_shape not found in data_dict")
        if not isinstance(data_dict["iterator_shape"], UdaoEmbedItemShape):
            iterator_shape_dict = data_dict["iterator_shape"]
    ...
    def hash(self) -> str:
        ...
        sha256_hash = hashlib.sha256(attributes_tuple)
        hex12 = sha256_hash.hexdigest()[:12]
        return "tree_cnn_sk_" + hex12


def get_tree_cnn_sk_mlp(params: TreeCNNSKParams) -> UdaoModel:
    if (
        params.readout != "max"
        or params.output_size != 64
        or params.tcnn_hidden_dim != 256
...
def add_new_rows_for_series(series: pd.Series, fixed_value: int) -> pd.Series:
    df = series.groupby(level="plan_id").size().to_frame("operator_id").copy()
...
def add_new_rows_for_df(data: pd.DataFrame, fixed_values: List[float]) -> pd.DataFrame:
    ...
    df = data.groupby(level="plan_id").size().to_frame("operator_id").copy()
...
def save_divide(
    ...
    if bm == "job":
        sink_job_stats(ckp_learning_header, device, obj_names)
    elif bm.endswith("+job"):
        index_splits_jobs = PickleHandler.load(
            f"{base_dir}/cache_and_ckp/job_100000x1", "index_splits_q_compile.pkl"
        )
        ...
        sink_job_stats(ckp_learning_header, device, obj_names, job_mask)

        start = 0
        for bm_tpc in bm.split("+")[:-1]:
            bm_tpc = bm_tpc.split("_")[0]
            if bm_tpc == "tpch":
                bm_tpc_fold = None
                tpc_index_header = f"{base_dir}/cache_and_ckp/tpch_102x490"
            elif bm_tpc == "tpcds":
                bm_tpc_fold = fold
                tpc_index_header = f"{base_dir}/cache_and_ckp/tpcds_102x490"
            else:
                raise ValueError(f"Unknown benchmark {bm_tpc}")
            index_splits_tpc = PickleHandler.load(
                tpc_index_header,
                f"index_splits_q_compile-{bm_tpc_fold}.pkl"
                if bm_tpc_fold
                else "index_splits_q_compile.pkl",
            )
            if not isinstance(index_splits_tpc, dict):
                raise TypeError(f"Expected dict, got {type(index_splits_tpc)}")
            n_test_tpc = len(index_splits_tpc["test"])
            tpc_mask = np.array([False] * n_test)
            tpc_mask[start : start + n_test_tpc] = True
            sink_tpc_stats(bm_tpc, ckp_learning_header, device, tpc_mask)
            start += n_test_tpc
        if start + n_test_jobs != n_test:
            raise ValueError(f"Expected {n_test}, got {start + n_test_jobs}")
    elif bm == "tpcds+tpch":
        start = 0
        for bm_tpc in bm.split("+"):
            bm_tpc = bm_tpc.split("_")[0]
            if bm_tpc == "tpch":
                bm_tpc_fold = None
                tpc_index_header = f"{base_dir}/cache_and_ckp/tpch_102x490"
            ...
            start += n_test_tpc
        if start != n_test:
            raise ValueError(f"Expected {n_test}, got {start}")
...
def save_query_embeddings(
    ...
    for split, iterator in split_iterators.items():
        if split == "train":
            ...
            iterator.set_augmentations([])
        iterator = cast(QueryPlanIterator, iterator)
        emb_file_name = f"query_embedding_{split}_{device}.pkl"
        if os.path.exists(f"{ckp_learning_header}/{emb_file_name}"):
            try:
                emb_df = PickleHandler.load(ckp_learning_header, emb_file_name)
                print("find cached results")
                if not isinstance(emb_df, pd.DataFrame):
                    raise TypeError("cache is not a emb_df (DataFrame)")
                continue
            except Exception as e:
                print("the cache is not ready due to: ", e)
        print(f"not found query embedding for {split}, start creating...")
        dataloader = iterator.get_dataloader(
            batch_size=5000,
            num_workers=num_workers,
            shuffle=False,
        ...
        emb_cols = [f"emb-{i}" for i in range(embedding.shape[1])]
        emb_df = pd.DataFrame(embedding, columns=emb_cols)
        emb_df.index = iterator.objectives.data.index
        PickleHandler.save(
            emb_df,
            ckp_learning_header,
            emb_file_name,
...
def local_p99_err(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    return float(np.percentile(np.abs(y_pred - y_true), 99))


def local_p99_wape(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    return local_p99_err(y_true, y_pred) / np.mean(y_true)


def local_max_err(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    return float(np.abs(y_pred - y_true).max())


def local_max_wape(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    return local_max_err(y_true, y_pred) / np.mean(y_true)
