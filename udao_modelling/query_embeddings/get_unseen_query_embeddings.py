...
def main(
    ...
    unseen_queries_path: Annotated[str, typer.Option(help="Path of q_compile file to evaluate")],
    fold: Annotated[
    ...
    # add the unseen templates

    if not unseen_queries_path:
        unseen_queries = pd.read_parquet(
            "/home/guillaume/data/udao-spark-optimizer/data/tpcds-ext-unseen-snowflake/parquet_2051x10/df_q_compile.parquet"
        )
    else:
        logger.warning("Loading parquet of unseen queries")
        unseen_queries = pd.read_parquet(unseen_queries_path)
        logger.warning("Loading of parquet completed")
    full_df = unseen_queries.reset_index()
    full_df.set_index("id", inplace=True, drop=False)
    dp = PickleHandler.load(path_watcher.cc_extract_prefix, "data_processor.pkl")
    iterator = dp.make_iterator(
        keys=full_df.index.to_list(), data=full_df, split="test"
    )
    split_iterators["unseen_templates"] = iterator  #  type: ignore

    logger.warning("Generating dataloader for unseen queries")
    ...
    all_embeddings = []
    logger.warning("Generating embeddings for unseen queries")
    ...
    logger.warning("Writing embeddings to disk")
    pq.write_table(
        embedding_table,
